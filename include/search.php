...
if ($mode == "searchlist") {
    if ($_REQUEST['chk'] == "yes") {
        $sql = "select * from placing_item_bid where  bid_starting_date and status='Active' and selling_method!='want_it_now' and selling_method!='ads' and ";
        if ($_REQUEST['chkitemspriced']) {
            ...
            $sql.=" (cur_price >= $max_cur and cur_price <= $min_cur) and ";
        }
    ...
    $sql = rtrim($sql, " and ") . " and expire_date ";
    /* echo $sql;
      echo "<br>"; */
}
...
if ($mode == "linkmode") {
    $_SESSION['catt'] = "";
    $sql = "select * from placing_item_bid where  bid_starting_date and status='Active' and selling_method!='ads' and selling_method!='want_it_now' and (";
    if ($category_id) {
...
if ($mode == "keysearch") {
    if (!empty($key_word)) {
        ...
        $sql = "select * from placing_item_bid  where status=\"Active\" and selling_method!=\"want_it_now\" and  (item_title like \"%$key_word%\" or detailed_descrip like \"%$key_word%\" or  item_id=\"$key_word\" or user_id=\"$user_id\") and bid_starting_date and ";
    } else {
        $sql = "select * from placing_item_bid where status=\"Active\" and selling_method!=\"want_it_now\" and bid_starting_date <= now() and expire_date";
    }
    if (!empty($show)) {
        if ($show == "all")
...
if ($mode == "advanced") {
    ...
    $category_id = $_REQUEST['category_id'];
    if ($category_id == "all")
        $sql = "select * from placing_item_bid a, user_registration b where a.selling_method!=\"want_it_now\" and bid_starting_date  and a.expire_date and ";
    else {
        ...
        $sql = "select * from placing_item_bid a,user_registration b where a.selling_method!=\"want_it_now\" and ($cat) and bid_starting_date and a.expire_date>=now() and ";
    }
    if ($item_status == 1)
...
if ($mode == "easy") {
    ...
    if (!empty($cat_id)) {
        ...
        elseif ($_REQUEST[show_all] == "wanted")
            $sql = "select * from placing_item_bid  where  status=\"Active\" and  selling_method = \"wanted\" and bid_starting_date and selling_method!=\"want_it_now\" ";
            ...
            if ($cat)
                $sql = "select * from placing_item_bid  where  selling_method!=\"want_it_now\" and status=\"Active\" and ($cat) and bid_starting_date and expire_date ";
        }
        if (!empty($prd)) {
            $prd = str_replace('\"', '', $prd);
            $prd = trim($prd);
            ...
            if ($cat_id == "all")
                $sql.="where status=\"Active\" and selling_method!=\"want_it_now\" and bid_starting_date and";
            $sql.=" (item_title like \"%$prd%\" or detailed_descrip like \"%$prd%\") and expire_date ";
        }
        if ($cboprice) {
        ...
        if (strlen($sql) != 32)
            ...
            $sql = "select * from placing_item_bid where status='Active' and selling_method!='want_it_now' and bid_starting_date  and expire_date ";
    }
}
...
if ($mode == 'sellers_item') {
    ...
    $sql = "select * from placing_item_bid where status='Active' and selling_method!='want_it_now' and bid_starting_date  and expire_date  and user_id=" . $sellers_userid;
}
