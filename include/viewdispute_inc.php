...
if($type=="unpaid")
{
 //$dispute_sql="select * from disputeconsole where ( dispute_to=$userid or dispute_by=$userid ) and dispute_type='unpaid' and dispute_status!='closed' group by dispute_id order by dispute_id desc";
 $dispute_sql="select * from disputeconsole where ( dispute_to=$userid or dispute_by=$userid ) and dispute_type='unpaid' order by dispute_id desc";
}
 else
{
 // $dispute_sql="select * from disputeconsole a , placing_item_bid b ,placing_bid_item c where (dispute_to=$userid or dispute_by=$userid) and dispute_type='notreceived' and dispute_status!='closed' group by dispute_id order by dispute_id desc";
 $dispute_sql="select * from disputeconsole a , placing_item_bid b ,placing_bid_item c where (dispute_to=$userid or dispute_by=$userid) and dispute_type='notreceived' order by dispute_id desc";
} 
//echo "$dispute_sql";
//echo "<br>";
