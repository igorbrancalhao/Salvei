...
function reduce_image($file, $filewidth, $fileheight, $file_path) {
    ...
    if ($width > $filewidth) {
        switch ($type) {
            case 1: $src = imagecreatefromgif($uploadedfile);
                break;
            case 2: $src = imagecreatefromjpeg($uploadedfile);
                break;
            case 3: $src = imagecreatefrompng($uploadedfile);
                break;
        ...
        switch ($type) {
            case $image_type['gif']: $ret = imagegif($tmp, $file_path, 100);
                break;
            case $image_type['jpg']: $ret = imagejpeg($tmp, $file_path, 100);
                break;
            case $image_type['png']: $ret = imagepng($tmp, $file_path);
                break;
        ...
        $newwidth = $width;
        $newheight = $height; //($height/$width)*$size;
        $tmp = imagecreatetruecolor($newwidth, $newheight) or die("Cannot Initialize new GD image stream");
