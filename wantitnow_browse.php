<?php
/***************************************************************************
 *File Name				:wantitnow_browse.php
 ...
 function ret($ssid)
{
 ...
 while($cat_row=mysql_fetch_array($sub_res))
 {
  ...
  $count_sql="select * from placing_item_bid where category_id=$cat_row[category_id] and selling_method='want_it_now' and status='Active' and bid_starting_date <= now() and expire_date>=now()";
 ...
 <tr><td class="detail9txt"><a href="category.php?cate_id=<?php=$cat_row['category_id']; ?>&view=list&mode=want" class="detail7txt">
 <font size=1 face="Arial"><?php=$cat_row['category_name']; ?></font></a>&nbsp;( <?php= $count_num ?> )</td></tr>	
 <?php
  $ssid=$cat_row['category_id'];
  ret($ssid);
 }	
 ...
 if($tot_rec == 0)
{
echo '<meta http-equiv="refresh" content="0;url=category.php?cate_id='.$cat_id.'&view=list&mode=want">';
exit();
}

require 'templates/whatitnow_browse.tpl';
