...
<tr><td bgcolor="#cccccc" class="txt_users" colspan="2"><?php echo ucwords($mode); if($type=='d') echo ' Default' ?> Footer Banner(728X90)</td></tr>
...
<?php		
		...
		if($type=='e') {
			if(!$mode) $mode='a';
			if($mode=='s') $ban_sql="select * from banners where status <> 'default' and status='off'";
			else if($mode=='a') $ban_sql="select * from banners where status <> 'default' and status='on'";
		}
		else if($type=='d') $ban_sql="select * from banners where status = 'default' or status='ad'";
...
?>
<tr bgcolor="#eeeee1">
<!--<td><?php if($type=='e') echo "<input type='checkbox' name='chkMain' value=1 onClick='selectall()'>";else echo '&nbsp;'?></td>-->
...
?>
<!--<tr bgcolor="#eeeee1">
  <td colspan="6" align="center"><input type="submit" name="<?php if($mode=='s' || $type=='d') echo 'btn_Act';else echo 'btn_Sus';?>" value="<?php if($mode=='s' || $type=='d') echo 'Activate';else echo 'Suspend';?>" class="button"></td>-->
</tr>
<?php
/*}*/
?>
