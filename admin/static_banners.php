...
$special_char=array('*','#','@','!','%','&','|','+','-','$','^');
	...
	if($mode=='edit') {
	$remove=$_GET['remove'];
		$edit_sql="select * from staticbanners where banner_id=$id";
		$edit_res=mysql_query($edit_sql);
		$edit_row=mysql_fetch_array($edit_res);	
		if($remove==1)
		{
			  unlink($edit_row['banner_path']);
			  $up_ban_sql="update staticbanners set banner_path='null' where banner_id=$id";
...
<?php
	...
	$f=0;
			...
			if(($mode=='add') and ($f!=1))
			{
			$ins_sql="insert into staticbanners(banner_name,banner_path,site_url,status) values('$bname','$burl','$surl','$status')";
			$msg=1;
			}
			...
			if($burl)
			{
			$ins_sql="update staticbanners set banner_name='$bname',banner_path='$burl' where banner_id=$id";
			$msg=2;
			}
			else
			{
		   $ins_sql="update staticbanners set banner_name='$bname' where banner_id=$id";
		   $msg=2;
			
			}
			}
			$ins_res=mysql_query($ins_sql);	
			echo '<meta http-equiv="refresh" content="0;url=static_banners.php?msg='.$msg.'>';
		}
...
<tr><td bgcolor="#cccccc" class="txt_users" colspan="2"><?php echo ucwords($mode); if($type=='d') echo ' Default' ?> Static banners</td></tr>
...
<?php
	if(($edit_row['banner_path']!='null')) {
?>
<tr bgcolor="#eeeee1">
  <td colspan="2">
<img src="../images/<?php=$edit_row['banner_path']?>"><br>
<div align="center"><a href="static_banners.php?mode=edit&id=<?php=$id?>&remove=1" id="tablink" onClick="return condel()">Remove</a></div>
...
?>
<!--<tr bgcolor="#eeeee1"><td>Site Url</td><td><input type="text" id="txtSurl"  name="txtSurl" value="<?php if($mode=='edit') echo $edit_row['site_url']; else echo 'http://';?>" size="60"> </td></tr>-->
<tr bgcolor="#eeeee1"><td colspan="2" align="center"><input type="hidden" name="type" value="<?php=$type?>">
<input type="hidden" name="canAdd" value="1">
...
<?php
	    ...
	    $del_sql="delete from staticbanners where banner_id=$id";
...
<?php		
	 ...
	 $ban_sql="select * from staticbanners";
...
<tr><td colspan="6" bgcolor="#cccccc" class="txt_users">Static Banners</td></tr>
<?php
	if(mysql_num_rows($ban_result)>0) {

?>
<tr bgcolor="#eeeee1">
<td><b>Banner Name</b></td><!--<td><b>Path</b></td>-->
...
?>
<tr bgcolor="#eeeee1">
<td><?php=$row['banner_name']?></td>
<!--<td><?php=$row['site_url']?></td>-->
<td><a href="static_banners.php?mode=edit&id=<?php=$row['banner_id']?>" id="tablink">Edit</a></td>
</tr>
<?php
	$i+=1;
	}
