...
<?php		
		...
		if($type=='e') {
			if(!$mode) $mode='a';
			if($mode=='s') $ban_sql="select * from small_banner where status <> 'enable'";
			else if($mode=='a') $ban_sql="select * from small_banner where status <> 'enable' ";
		}
		else if($type=='d') $ban_sql="select * from small_banner where status = 'enable' ";
