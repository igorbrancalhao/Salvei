...
$special_char=array('*','#','@','!','%','&','|','+','-','$','^');
	...
	if($mode=='edit') {
		...
		$remove=$_GET['remove'];
		$edit_sql="select * from frontpage_banner where id=$id";
		$edit_res=mysql_query($edit_sql);
		$edit_row=mysql_fetch_array($edit_res);	
		if($remove==1)
		{
			  unlink($edit_row['banner_path']);
			  $up_ban_sql="update frontpage_banner set banner='null' where id=$id";
	...
	if($canadd==1) {
		$f=0;
			...
			$status='off';	
					...
					if(($height>259) or ($width>278))
					{
						$f=1;
						$msg="Please Enter the Correct Banner Image Size(278x259)";
					}
					...
					else
				   {
					...
					$surl=$_POST['txtSurl'];
			if(($mode=='add') and  ($f!=1))
			$ins_sql="insert into frontpage_banner(banner,url,status) values('$burl','$surl','enable')";
			else if(($mode=='edit') and ($f!=1))
			$ins_sql="update frontpage_banner set banner='$burl',url='$surl',status='$status' where id=$id";
			$ins_res=mysql_query($ins_sql);
			if($ins_res)
			echo '<meta http-equiv="refresh" content="0;url=frontpagebanner.php?msg=a">';
				}
...
<table>
<tr><td width=93>
<table id="Table_01" width="91" border="0" cellpadding="0" cellspacing="0">
                  <tr>
...
<tr>
  <td bgcolor="#cccccc" colspan="2"><b>Add Front Page(278X259) Banner</b></td>
</tr>
...
<div align="center"><a href="frontpagebanner.php?mode=edit&id=<?php=$id?>&remove=1" id="tablink" onClick="return condel()">Remove</a></div>
...
?>
<tr bgcolor="#eeeee1"><td>Site Url</td><td><input type="text" name="txtSurl" value="<?php if($mode=='edit') echo $edit_row['site_url']; else echo 'http://';?>" size="60"> </td></tr>
...
<?php
	    ...
	    $del_sql="delete from frontpage_banner where id=$id";
		$del_res=mysql_query($del_sql);
		echo '<meta http-equiv="refresh" content="0;url=frontpagebanner.php?msg=d">';
	}
	else {
...
<tr>
<td align="center"><a href="frontpagebanner.php?type=e&mode=s" id="tablink">Suspend</a></td>
<td align="center"><a href="frontpagebanner.php?type=e&mode=a" id="tablink">Activate</a></td>
</tr>
</table><br>
<?php		
	}
		if(isset($_POST['btn_Act'])) {
			if($type=='e') {
				$act=$_POST['chkAct'];
				foreach($act as $id) {
					$up_sql="update frontpage_banner set status='on' where id=$id";	
					$up_res=mysql_query($up_sql);
				}
			}
			else if($type=='d') {
				$act=$_POST['rdAct'];
				$up_sql="update frontpage_banner set status='ad' where id=$act";
				$up_res=mysql_query($up_sql);
				$up_sql1="update frontpage_banner set status='default' where id <> $act and status='ad'";
			...
			if($type=='e') {
				$act=$_POST['chkAct'];
				foreach($act as $id) {
					$up_sql="update frontpage_banner set status='off' where id=$id";	
					$up_res=mysql_query($up_sql);
				}
			}
			else if($type=='d') {
				$act=$_POST['rdAct'];
				$up_sql="update frontpage_banner set status='off' where id=$act";
					$up_res=mysql_query($up_sql);
			}
		}
		if($type=='e') {
			if(!$mode) $mode='a';
			if($mode=='s') $ban_sql="select * from frontpage_banner where status <> 'enable'";
			else if($mode=='a') $ban_sql="select * from frontpage_banner where status <> 'enable' ";
		}
		else if($type=='d') $ban_sql="select * from frontpage_banner where status = 'enable' ";
...
<tr>
  <td colspan="6" bgcolor="#cccccc" class="txt_users">Front Page(278X259) Banner Details</td>
</tr>
<tr bgcolor="#eeeee1">
...
<td> <img src="../<?php=$row['banner']?>" width="250" height="250"> </td>
 <td colspan="2"><a href="frontpagebanner.php?mode=del&id=<?php=$row['id']?>" id="tablink" onClick="return condel()">Delete</a></td>
</tr>
<?php
	$i+=1;
	}
...
<tr bgcolor="#eeeee1">
<td colspan="6"><a href="frontpagebanner.php?mode=add&type=<?php=$type?>" id="tablink">Add New</a></td>
</tr>
</form>
</table></td></tr></table></td></tr></table>
<?php
...
function validate() {
	...
	if(len1>1) {
	for(i=0;i<len1;i++) {
		if(document.frmBan.chkAct[i].checked==true) cval=cval+1;
	}
	}
	else {
		if(document.frmBan.chkAct.checked==true) cval=cval+1;
	}
	if(cval>=1) {
		return true;
	}
...
function selectall() {
	...
	if(len>1) {
		for(i=0;i<len;i++) {
			if(document.frmBan.chkMain.checked==true) document.frmBan.chkAct[i].checked=true;
			else document.frmBan.chkAct[i].checked=false;
		}
	}
	else {
			if(document.frmBan.chkMain.checked==true) document.frmBan.chkAct.checked=true;
			else document.frmBan.chkAct.checked=false;
	}
}
function condel() {
