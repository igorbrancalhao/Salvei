...
if($val=='exp')
$sql="select * from placing_item_bid where status='Closed' and selling_method='auction' and quantity_sold=0";
else if($val=='today')
$sql="select * from placing_item_bid  where TO_DAYS((bid_starting_date))=TO_DAYS( NOW()) and selling_method='auction' ";
else if($val=='actv')
$sql="select * from placing_item_bid  where status='Active' and selling_method='auction' ";
else if($val=='sold')
$sql="select * from placing_item_bid where quantity_sold > 0 and selling_method='auction'";
else if($val=='unpaid')
$sql="select * from placing_item_bid where status='new' and selling_method='auction'";
$res=mysql_query($sql);
$id=$_POST['id'];
...
<tr><td><br></td></tr>
 ...
 <?php if($msg)
  {
  ?>
 <table width="100%" bgcolor="#f7f7f7"> <tr><td colspan="2"><br> <?php=$msg;?> </td></tr></table>
  <?php
  $msg="";
  }
  ?>
  ...
  <form name="frm1" method="post" action="<?php $_SERVER['PHP_SELF']; ?>"> 
    <tr> 
      <td align="center"><a href="auction.php?val=today" class="txt_users">New Items</a></td> 
      <td align="center"><a href="auction.php?val=actv" class="txt_users">Live Items</a></td> 
	  <td align="center"><a href="auction.php?val=unpaid" class="txt_users">Unpaid Items</a></td> 
      <td align="center"><a href="auction.php?val=sold" class="txt_users">Sold Items</a></td> 
      <td align="center"><a href="auction.php?val=exp" class="txt_users">Expired Items</a></td> 
    </tr> 
  </table> 
  <table align="center" width="98%" class="border2" bgcolor="#f7f7f7"> 
        <tr bgcolor="#CCCCCC"><td height="24" colspan="8" align="center"><b>Simple Auction</b></td></tr>
    <tr bgcolor="#eeeee1"> 
      <td height="24" colspan="8" align="center"><b>
	  <?php if($val=='today') echo 'New ';else if($val=='actv') echo 'Live ';else if($val=='sold') echo 'Sold';else if($val=='exp') 																																																																										        echo 'Expired '; else if($val=='unpaid')echo "Unpaid";?> Items</b>
	  </td> 
    </tr> 
    <tr bgcolor="#CCCCCC" class="style1"> 
	  ...
	  <td width="23%">
	  <?php if($val!="sold") echo "Current Price"; else if($val=="sold") echo "Sold Amount"; ?> </td> 
      ...
      <td width="11%" nowrap="nowrap">No. of Bids</td> 
       <td width="21%">No. of Clicks</td> 
	  <?php if($val!="sold")
	  {
	  ?> 
 ...
 $sql="select * from placing_item_bid where ( $cat ) and status='Active' and selling_method='auction'";
 }
 if($adate!='' && $adate!='--')
...
?> 
	...
	if($curpage!=1) {
?> 
        <a href="auction.php?curpage=<?php=($curpage-1);?>&val=<?php=$val?>" id="link2">Prev</a> |
        <?php
}
?> 
        <?php
if($tot > ($start + $end)) {
?> 
        <a href="auction.php?curpage=<?php=($curpage+1);?>&val=<?php=$val?>" id="link2">Next</a> 
        <?php
}
?> </td> 
    </tr> 
...
?> 
      ...
      <td align="center">
	  	<a href="item_details.php?id=<?php= $row[item_id]?>" id="link3"><?php echo $row['item_title'];?></a>
	  </td> 
...
<?php  
	...
	{
	$cur_sql1="select * from placing_bid_item where user_pos='Yes' and item_id=".$row['item_id'];
    $cur_res1=mysql_query($cur_sql1);
    $cur_row1=mysql_fetch_array($cur_res1);
	echo $cur_row1['bidding_amount'];  
	}
	?>
</td>
<td align="center"><a href="userdetails.php?id=<?php= $row[user_id] ?>&type=gen" id="link3">
<?php
...
<td align="center">
     <?php 
	  ...
	  {
	  ?>
	  <a href="auction.php?item_id=<?php= $row['item_id']; ?>&mode=close" id="link3"> <?php= "Close" ?> </a>
	  <?php
	  }
	   else if($row[status]=="Suspend")
	   {
	  ?>
	   <a href="auction.php?item_id=<?php= $row['item_id']; ?>&mode=Active" id="link3"> <?php= "Active" ?> </a>
	  <?php
	  } 
	  ...
	  {
      ?>
	   <a href="auction.php?item_id=<?php= $row['item_id']; ?>&mode=paid" id="link3"> Paid </a>
	  <?php
	  }
	  ?>
...
?> 
    ...
    <tr bgcolor="#eeeee1"> 
      <td colspan="8"> 
	  <input type="submit" name="<?php if($val=='new') echo 'btn_new'; else if($val=='mature') echo 'btn_mature'; else echo 'btn_release'; ?>" value="<?php if($val=='new') echo 'Activate'; else  echo 'Delete';  ?>" class="button"></td> 
    </tr> 
    <?php } ?> 
    <?php
}
