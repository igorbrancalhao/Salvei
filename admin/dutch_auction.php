...
if($val=='exp')
$sql="select * from placing_item_bid where status='Closed' and selling_method='dutch_auction'";
else if($val=='today')
$sql="select * from placing_item_bid  where TO_DAYS((bid_starting_date))=TO_DAYS( NOW( )) and selling_method='dutch_auction' ";
else if($val=='actv')
$sql="select * from placing_item_bid  where status='Active' and selling_method='dutch_auction' ";
else if($val=='sold')
$sql="select * from placing_item_bid where quantity_sold > 0 and selling_method='dutch_auction'";
else if($val=='unpaid')
$sql="select * from placing_item_bid where status='new' and selling_method='dutch_auction'";
...
<table align="center" width="100%" height="35" >
 ...
 <table width="100%"> <tr align="center"><td colspan="2"><br> <?php=$msg;?> </td></tr></table>
  ...
  <form name="frm1" method="post" action="<?php $_SERVER['PHP_SELF']; ?>"> 
    <tr> 
      <td align="center"><a href="dutch_auction.php?val=today" class="txt_users">New Items</a></td> 
      <td align="center"><a href="dutch_auction.php?val=actv" class="txt_users">Live Items</a></td> 
  	  <td align="center"><a href="dutch_auction.php?val=unpaid" class="txt_users">Unpaid Items</a></td> 
      <td align="center"><a href="dutch_auction.php?val=sold" class="txt_users">Sold Items</a></td> 
      <td align="center"><a href="dutch_auction.php?val=exp" class="txt_users">Expired Items</a></td> 
    </tr> 
  </table> 
  <table align="center" width="98%" class="border2"> 
   <tr bgcolor="#CCCCCC"><td height="24" colspan="8" align="center"><b>Dutch Auction</b></td></tr>
    <tr bgcolor="#CCCCCC"> 
      <td height="24" colspan="8" align="center"><b><?php if($val=='today') echo 'New ';else if($val=='actv') echo 'Live';else if($val=='sold') echo 'Sold ';else if($val=='exp') echo 'Expired ';else if($val=='unpaid')echo "Unpaid";?> Items</b></td> 
    </tr> 
...
$tot=mysql_num_rows($res);$curpage=$_GET['curpage'];
...
?> 
	...
	if($curpage!=1) {
?> 
        <a href="dutch_auction.php?curpage=<?php=($curpage-1);?>&val=<?php=$val?>" id="link2">Prev</a> |
        <?php
}
?> 
        <?php
if($tot > ($start + $end)) {
?> 
        <a href="dutch_auction.php?curpage=<?php=($curpage+1);?>&val=<?php=$val?>" id="link2">Next</a> 
        <?php
}
?> </td> 
    </tr> 
...
?> 
    <tr bgcolor="#eeeee1"> 
      <td align="center"><input name="chkSub[]" id="chkSub" type="checkbox" class="check" value="<?php echo $row['item_id'];  ?>">
...
<td align="center">
     <?php 
	  ...
	  {
	  ?>
	  <a href="dutch_auction.php?item_id=<?php= $row['item_id']; ?>&mode=close" id="link3"> <?php= "Close" ?> </a>
	  <?php
	  }
	   else if($row[status]=="Suspend")
	   {
	  ?>
	   <a href="dutch_auction.php?item_id=<?php= $row['item_id']; ?>&mode=Active" id="link3"> <?php= "Active" ?> </a>
	  <?php
	  } 
	  ...
	  {
      ?>
	   <a href="dutch_auction.php?item_id=<?php= $row['item_id']; ?>&mode=paid" id="link3"> Paid </a>
	  <?php
	  }
	  ?>
...
echo '<tr align=center bgcolor="#eeeee1"><td colspan=8><font color=#FF0000>No items Found</font></td></tr>';
?> 
</form>
  </table> 
