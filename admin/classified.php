...
if($val=='exp')
$sql="select * from placing_item_bid where status='Closed' and selling_method='ads'";
else if($val=='today')
$sql="select * from placing_item_bid  where TO_DAYS((bid_starting_date))=TO_DAYS( NOW( )) and selling_method='ads' ";
else if($val=='actv')
$sql="select * from placing_item_bid  where status='Active' and selling_method='ads' ";
else if($val=='sold')
$sql="select * from placing_item_bid where quantity_sold > 0 and selling_method='ads'";
else if($val=='unpaid')
$sql="select * from placing_item_bid where status='new' and selling_method='ads'";
...
<form name="frm1" method="post" action="<?php $_SERVER['PHP_SELF']; ?>"> 
  <table align="center" width="60%"> 
    <tr> 
      <td align="center"><a href="classified.php?val=today" class="txt_users">New Items</a></td> 
      <td align="center"><a href="classified.php?val=actv" class="txt_users">Live Items</a></td> 
	  <td align="center"><a href="classified.php?val=unpaid" class="txt_users">Unpaid Items</a></td> 
 <!--      <td align="center"><a href="classified.php?val=sold" id="tablink">Sold Items</a></td> -->
      <td align="center"><a href="classified.php?val=exp" class="txt_users">Expired Items</a></td> 
	  
    </tr> 
  </table> 
  <table align="center" width="98%" class="border2"> 
      <tr bgcolor="#CCCCCC"><td height="24" colspan="8" align="center"><b>Classified Ads</b></td></tr>  
    <tr bgcolor="#CCCCCC"> 
...
?> 
	...
	if($curpage!=1) {
?> 
        <a href="classified.php?curpage=<?php=($curpage-1);?>&val=<?php=$val?>" id="link2">Prev</a> |
        <?php
}
?> 
        <?php
if($tot > ($start + $end)) {
?> 
        <a href="classified.php?curpage=<?php=($curpage+1);?>&val=<?php=$val?>" id="link2">Next</a> 
        <?php
}
?> </td> 
    </tr> 
...
<td>
     <?php 
	  ...
	  {
	  ?>
	  <a href="classified.php?item_id=<?php= $row['item_id']; ?>&mode=close" id="link3"> <?php= "Close" ?> </a>
	  <?php
	  }
	   else if($row[status]=="Suspend")
	   {
	  ?>
	   <a href="classified.php?item_id=<?php= $row['item_id']; ?>&mode=Active" id="link3"> <?php= "Active" ?> </a>
	  <?php
	  } 
	  ...
	  {
      ?>
	   <a href="classified.php?item_id=<?php= $row['item_id']; ?>&mode=paid" id="link3"> Paid </a>
	  <?php
	  }
	  ?>
